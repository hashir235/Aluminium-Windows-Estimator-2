...
int D_Win::windowCount = 0; 
void D_Win::resetWindowCount() { windowCount = 0;}
D_Win::D_Win()
    : height(0.0), width(0.0), 
      collarType(0), winType(0), inputInInches(false) {++windowCount;}

bool D_Win::inputWindowData(bool isEditing) {
    if (!isEditing) {
        
        label = "D_Win #" + std::to_string(windowCount) + " -> ";
    } else {
        std::cout << "\n--- Editing " << label << " ---\n";
    }
...
switch (collarType) {
        ...
        case 2:
            ...
            break;
        case 3:
            addSection("D54F", { labelWith("HR", h + 0.25), labelWith("WT", w + 0.25) });
        ...
        case 7:
            ...
            addSection("D54A", { labelWith("HR", h), labelWith("WT", w ) });
            break;
        case 8:
            addSection("DC30C", { labelWith("WT", w) });
            addSection("DC30F", { labelWith("HL", h), labelWith("HR", h) });
            break;
    }   
    ...
    if(winType  == 1){ // Single Door
       ...
       if(bottom == "y" || bottom == "Y") {       
            addSection("D50", { labelWith("WT", w), labelWith("HL", h), labelWith("HR", h) });
            addSection("D46", { labelWith("WB", w) });
          if(Tee == "y" || Tee == "Y") {
               addSection("52", { labelWith("Tee", w) });
            }
        }else if(bottom == "n" || bottom == "N") {
            addSection("D50", { labelWith("WT", w), labelWith("WB", w), labelWith("HL", h), labelWith("HR", h) });
           if(Tee == "y" || Tee == "Y") {
               addSection("D52", { labelWith("Tee", w) });
            }
        }
        ...
        if(bottom == "y" || bottom == "Y") {       
            ...
            addSection("D50", { labelWith("WT_l", w/2), labelWith("WT_r", w/2),
                 labelWith("HL_l", h), labelWith("HR_l", h), labelWith("HL_r", h), labelWith("HR_r", h) });
            addSection("D46", { labelWith("WB_l", w/2), labelWith("WB_r", w/2) });
        if(Tee == "y" || Tee == "Y") { //Tee
            addSection("D52", { labelWith("Tee_l", w/2), labelWith("Tee_r", w/2)});
        }

        }else if(bottom == "n" || bottom == "N") {

            addSection("D50", { labelWith("WT_l", w/2), labelWith("WT_r", w/2), labelWith("WB_l", w/2), labelWith("WB_r", w/2),
                 labelWith("HL_l", h), labelWith("HR_l", h), labelWith("HL_r", h), labelWith("HR_r", h) });   
        if(Tee == "y" || Tee == "Y") { //Tee
            addSection("D52", { labelWith("Tee_l", w/2), labelWith("Tee_r", w/2)});
        }
...
double D_Win::getAreaSqFt() const {
    return height * width;
}

std::string D_Win::getLabel() const {
    return label;
}

const SectionMap& D_Win::getSectionMap() const {
    return sectionMap;
}


void D_Win::sendToEstimator(EstimateLengthManager& estimator) {
    estimator.addWindowSections(label, sectionMap);
}

char D_Win::getSize() const {return unitChoice;}
double D_Win::getHeight() const { if (inputInInches) {return height * 12.0;} return height;}
double D_Win::getWidth() const {if (inputInInches){return width * 12.0;} return width;}
int D_Win::getCollarType() const { return collarType; }


void D_Win::sendToEstimator() {
