...
int O_Win::windowCount = 0; 
void O_Win::resetWindowCount() { windowCount = 0;}
O_Win::O_Win() {
    height = 0.0;
    width = 0.0;
    collarType = 0;
...
bool O_Win::inputWindowData(bool isEditing) {
    if (!isEditing) {

        label = "O_Win #" + std::to_string(windowCount) + " -> ";
    } else {
        std::cout << "\n--- Editing " << label << " ---\n";
    }
...
void O_Win::calculateSections() {
    ...
    switch (collarType) {
        ...
        // Inner Sections
        addSection("D50", { labelWith("HL", h), labelWith("HR", h ), labelWith("WT", w ), labelWith("WB", w ) });

    if(netChoice == "y" || netChoice == "Y") {
        addSection("D29", { labelWith("HL", h), labelWith("HR", h), labelWith("WT", w), labelWith("WB", w) });
    }
}

double O_Win::getAreaSqFt() const {
    return height * width;
}

std::string O_Win::getLabel() const {
    return label;
}

const SectionMap& O_Win::getSectionMap() const {
    return sectionMap;
}


void O_Win::sendToEstimator(EstimateLengthManager& estimator) {
    estimator.addWindowSections(label, sectionMap);
}

char O_Win::getSize() const { return unitChoice; }
double O_Win::getHeight() const { if (inputInInches) {return height * 12.0;} return height;}
double O_Win::getWidth() const {if (inputInInches){return width * 12.0;} return width;}
int O_Win::getCollarType() const { return collarType; }


void O_Win::sendToEstimator() {
    std::cout << "[O_Win] Warning: No estimator provided in sendToEstimator().\n";
}
